...
const getAllCoupon = async (query: Record<string, unknown>) => {
     const brandQuery = new QueryBuilder(Coupon.find().populate('serviceCategory'), query).search(['code']).filter().sort().paginate().fields();
...
const getAllUnpaginatedCoupon = async (query: Record<string, unknown>) => {
     const brandQuery = new QueryBuilder(Coupon.find(), query).search(['code']).filter().sort().fields();
