 ...
 * @param {IBooking} booking - The order object to generate the invoice for.
 * @returns {Promise<Buffer>} - The generated PDF as a Buffer.
 */
export const generateBookingInvoicePDF = async (booking: IBooking): Promise<Buffer> => {
    return new Promise<Buffer>(async (resolve, reject) => {
        try {
            ...
            // Invoice Details
            doc.fontSize(11).fillColor('#000000').text(`Invoice ID: ${booking._id}`);
            doc.text(`Order Date: ${(booking.createdAt as Date).toLocaleDateString()}`);
            doc.moveDown(0.5);
            //@ts-ignore
            doc.text(`Customer Id: ${booking.user}`);
            doc.text(`Shipping Address: ${booking.servicingDestination}`);
            doc.moveDown(1);
            ...
            booking.services.forEach((item) => {
                //@ts-ignore
                const productName = String(item.service) || 'Unknown Product';
                const quantity = item.quantity;
                //@ts-ignore
                const price = item.serviceCharge * quantity || 0;
            ...
            doc.text(`${booking.finalAmount.toFixed(2)} /-`, 400, pricingY, { width: 90, align: 'right' });
            pricingY += tableHeight;

            doc.fontSize(11).fillColor('#000000').text('Discount', 50, pricingY, { width: 200 });
            doc.text(`-${booking.discount.toFixed(2)} /-`, 400, pricingY, { width: 90, align: 'right' });
            pricingY += tableHeight;

            // Final Amount (Bold and Color)
            doc.fontSize(11).font('Helvetica-Bold').fillColor('#003366').text('Total', 50, pricingY, { width: 200 });
            doc.fontSize(11).font('Helvetica-Bold').fillColor('#003366').text(`${booking.finalAmount.toFixed(2)} /-`, 400, pricingY, { width: 90, align: 'right' });
            pricingY += tableHeight;
